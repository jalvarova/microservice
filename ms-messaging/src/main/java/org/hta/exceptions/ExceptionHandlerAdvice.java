...
public class ExceptionHandlerAdvice {
    ...
    public ResponseEntity<?> errorIO(IOException ex, HttpServletRequest req) {
        log.error("Exception error handler " + "errorIO" + " error http : " + INTERNAL_SERVER_ERROR);

        ErrorResponse errorHandler =
                ErrorResponse
                        .builder()
                        .message("Error generico de servidor")
                        .exception(ex.getMessage())
                        .error("ERR-203")
